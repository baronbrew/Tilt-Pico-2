...
# Example for a 16-byte key: b'SixteenByteKey!!'
AES_KEY_BYTES = b'SixteenByteKey!!' # <--- REPLACE WITH YOUR EXACT KEY BYTES

# Paste the Base64-encoded encrypted string from the JavaScript output here
ENCRYPTED_STRING_BASE64 = "HasOu0Zy4PkbiZxxo5kPmzyPJw1H0XrI4tdz/YcgmhO+OmltM9UHTMG49+Y/eGW36G2zLvVLbW3GyVVdWn+keg==" # <--- REPLACE THIS WITH YOUR ENCRYPTED STRING
HasOu0Zy4PkbiZxxo5kPmzyPJw1H0XrI4tdz/YcgmhO+OmltM9UHTMG49+Y/eGW36G2zLvVLbW3GyVVdWn+keg==
...
# --- AES Decryption ---
def decrypt_aes_cbc(encrypted_base64_string, key_bytes):
    """
...
if __name__ == "__main__":
    if not ENCRYPTED_STRING_BASE64:
        print("Error: No encrypted string provided. Paste the Base64 string into ENCRYPTED_STRING_BASE64.")
    else:
        print("Attempting to decrypt the string using AES-CBC...")
        decrypted_result = decrypt_aes_cbc(ENCRYPTED_STRING_BASE64, AES_KEY_BYTES)

        if decrypted_result:
            print("\n--- Decryption Successful ---")
            print("Decrypted Plaintext:", decrypted_result)
            print("---------------------------")
        else:
            print("Decryption failed. Please check the key, encrypted string, or padding.")